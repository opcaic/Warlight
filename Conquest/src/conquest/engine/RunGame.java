...
public class RunGame
{
	...
	{ 
		try {
			...
			robots[0] = setupRobot(1, config.bot1Init, config.bot1JarPath);
			robots[1] = setupRobot(2, config.bot2Init, config.bot2JarPath);
	...
	private Robot setupRobot(int player, String botInit, String botJarPath) throws IOException {
		...
		if (botInit.startsWith("external:")) {
			String botFQCN = botInit.substring(9);
			return new InternalRobot(player, botFQCN, botJarPath);
		}
